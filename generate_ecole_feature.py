...
def make_samples(in_queue, out_queue, stop_flag):
    ...
    while not stop_flag.is_set():
        ...
        scip_parameters = {"presolving/maxrounds": 0}
        env = Environment(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['item_placement', 'load_balancing', 'anonymous', 'corlat', 'mik'],
    )
    parser.add_argument(
        '-s', '--seed',
    ...
    #     out_dir = 'train_files/samples/1_item_placement_2019'
    #
    ...
    #     out_dir = 'train_files/samples/2_load_balancing_2019'
    #
    ...
    #     out_dir = 'train_files/samples/3_anonymous_2019'
    if args.problem == "corlat":
        instances_train = glob.glob("/data/corlat/train/*.mps")
        instances_valid = glob.glob("/data/corlat/test/*.mps")
        out_dir = 'train_files/samples/colat'
        # train_size = min(train_size, len(instances_train))
        # valid_size = min(valid_size, len(instances_valid))
    elif args.problem == "mik":
        instances_train = glob.glob("/data/mik/train/*.mps")
        instances_valid = glob.glob("/data/mik/test/*.mps")
        out_dir = 'ecole_features/mik'
        # train_size = min(train_size, len(instances_train))
        # valid_size = min(valid_size, len(instances_valid))
    elif args.problem == "item_placement":
        instances_train_ = glob.glob("/data/1_item_placement/train/*.mps.gz")
        solved_path = "results/reward3_annealing_results_hard_nips_item_placement.csv"
        df = pd.read_csv(solved_path, header=None)
        solved_data = df[0].tolist()
        instances_train = solved_data
        train_size = len(instances_train) * 10
        instances_valid = glob.glob("/data/1_item_placement/valid/*.mps.gz")
        out_dir = 'ecole_features/item_placement'
        # train_size = min(train_size, len(instances_train))
        # valid_size = min(valid_size, len(instances_valid))
    elif args.problem == "load_balancing":
        instances_train_ = glob.glob("/data/2_load_balancing/train/*.mps.gz")
        solved_path = "results/reward1_annealing_results_hard_nips_load_balancing.csv"
        df = pd.read_csv(solved_path, header=None)
        solved_data = df[0].tolist()
        instances_train = solved_data
        train_size = len(instances_train) * 10
        instances_valid = glob.glob("/data/2_load_balancing/valid/*.mps.gz")
        out_dir = 'ecole_features/load_balancing'
        # train_size = min(train_size, len(instances_train))
        # valid_size = min(valid_size, len(instances_valid))
    elif args.problem == "anonymous":
        instances_train_ = glob.glob("/data/3_anonymous/train/*.mps.gz")
        solved_path = "results/reward3_annealing_results_hard_nips_anonymous.csv"
        df = pd.read_csv(solved_path, header=None)
        solved_data = df[0].tolist()
        instances_train = solved_data
        train_size = len(instances_train) * 10
        print(train_size)
        out_dir = 'ecole_features/anonymous'
        instances_valid = glob.glob("/data/3_anonymous/valid/*.mps.gz")
        valid_size = min(valid_size, len(instances_valid))
    else:
